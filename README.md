...
Состояние хранится через асинхронный repository-слой; текущий адаптер использует `localStorage` браузера. Короткие видео-инструкции упражнений хранятся отдельно в IndexedDB того же браузера. Компактный переключатель `DEMO` меняет роль без сброса данных. В настройках доступно восстановление исходного seed и локальных видео.
