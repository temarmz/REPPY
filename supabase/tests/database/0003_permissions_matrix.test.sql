...
select is((select count(*)::integer from public.exercise_definitions), 28, 'student sees only the system exercise library');
...
select is((select count(*)::integer from public.exercise_definitions), 29, 'trainer sees the system library and only their own custom exercise');
